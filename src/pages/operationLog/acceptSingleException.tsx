...
/** 受理单异常日志 */
class AcceptSingleException extends React.Component<IProps, IState> {
    constructor (props: IProps) {
        super(props);
        this.state = {
            searchVal: {
                type: 'acceptSingleException',
            },
            total: 0,
            tableData: [],
    ...
    hanldeSearch = async () => {
        ...
        if (!searchData.type) {
            searchData.type = 'acceptSingleException';
        }
        const res: any = await getOperationLog(searchData);
        this.setState({ tableData: res.operationAcceptOrderExceptionLogList, total: res.count, loading: false });
    }
    render (): React.ReactNode {
        const { searchVal, total, tableData, defaultMomentValue, loading } = this.state;
        return <div className="abnormal-purchase-order">
            <LogSearch defaultMomentValue={defaultMomentValue} searchKeyList={OPERATION_LOG_DATA.ACCEPT_SINGLE_EXCEPTION_SEARCH_LIST} onSearch={this.onSearch}></LogSearch>
            <LogTable rowKey='id' loading={loading} tableData={tableData} tableTitle={OPERATION_LOG_DATA.ACCEPT_SINGLE_EXCEPTION_TABLE_ALL_LIST}></LogTable>
            <LogPaging total={total} changePage={this.changePage} pageNo={searchVal.pageNo || 1}></LogPaging>
        </div>;
    }
}

export default AcceptSingleException;
