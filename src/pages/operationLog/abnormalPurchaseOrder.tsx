...
/** 采购单异常日志 */
class AbnormalPurchaseOrder extends React.Component<IProps, IState> {
    constructor (props: IProps) {
        super(props);
        this.state = {
            searchVal: {
                type: 'abnormalPurchaseOrder',
            },
            total: 0,
            tableData: [],
    ...
    hanldeSearch = async () => {
        ...
        if (!searchData.type) {
            searchData.type = 'abnormalPurchaseOrder';
        }
        const res: any = await getOperationLog(searchData);
        if (!res) {
            this.setState({ loading: false });
        }
        this.setState({ tableData: res.operationPurchaseOrderExceptionLogList, total: res.count, loading: false });
    }
    render (): React.ReactNode {
        const { searchVal, total, tableData, defaultMomentValue, loading } = this.state;
        return <div className="abnormal-purchase-order">
            <LogSearch defaultMomentValue={defaultMomentValue}  searchKeyList={OPERATION_LOG_DATA.ABNORMAL_PURCHASE_ORDER_SEARCH_LIST} onSearch={this.onSearch}></LogSearch>
            <LogTable loading={loading} rowKey='id' tableData={tableData} tableTitle={OPERATION_LOG_DATA.ABNORMAL_PURCHASE_ORDER_TABLE_ALL_LIST}></LogTable>
            <LogPaging total={total} changePage={this.changePage} pageNo={searchVal.pageNo || 1}></LogPaging>
        </div>;
    }
}

export default AbnormalPurchaseOrder;
